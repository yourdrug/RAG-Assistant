...
export function AdminSettingsPage() {
    ...
    const {data: openrouterData, isLoading: isLoadingOpenRouter} = useQuery({
        ...
        queryFn: async () => (await apiClient.get<{ models: OpenRouterModel[] }>("/admin/models/openrouter")).data,
        enabled: (() => {
    ...
    const renderValue = (p: ConfigParam) => {
        ...
        if (p.value_type === "str" && p.key === "llm_model" && currentProvider === "ollama" && modelsInfo?.ollama_models) {
            return (
                <select
                    value={displayValue}
                    onChange={(e) => setEdits((prev) => ({...prev, [p.key]: e.target.value}))}
                        ...
                        >
                            {openrouterData.models.map((m) => (
                                <option key={m.id} value={m.id}>
                                    {m.name} ({m.context_length.toLocaleString()} ctx, ${m.pricing.prompt.toFixed(2)}/M)
                                </option>
                            ))}
                        </select>
