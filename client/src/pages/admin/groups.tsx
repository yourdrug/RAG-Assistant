...
import type { GroupMemberResponse, GroupResponse } from "@/shared/api/types";
...
export function AdminGroupsPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      await createMut.mutateAsync({ name: gName });
  ...
  const handleAdd = async () => {
    ...
    try {
      await addMut.mutateAsync({ groupId: selGroup, userId: Number(addUserId) });
      toast.success("Added");
      setAddUserId("");
    } catch {
      toast.error("Failed");
  ...
  const handleRemove = async (uid: number) => {
    if (!selGroup) return;
    try {
      await removeMut.mutateAsync({ groupId: selGroup, userId: uid });
  ...
  const columns: ColumnDef<GroupResponse>[] = [
    ...
    },
    {
      ...
      header: "",
      cell: ({ row }) => (
        <Button variant="ghost" size="sm" onClick={() => setSelGroup(row.original.id)}>
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle>{selGroup ? `Members (Group #${selGroup})` : "Select a group"}</CardTitle>
          </CardHeader>
          <CardContent>
            {selGroup ? (
              <div className="space-y-4">
                ...
                {members && members.length > 0 ? (
                  <div className="space-y-2">
                    {members.map((m: GroupMemberResponse) => (
                      <div
                        key={m.id}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Add a new user group</DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
