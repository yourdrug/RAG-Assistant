...
export function AdminClientsPage() {
  ...
  const handleAssign = async () => {
    ...
    try {
      await assignMut.mutateAsync({
        clientUserId: Number(selClient),
        internalUserId: Number(selInternal),
      });
  ...
  const handleUnassign = async (uid: number) => {
    if (!selClient) return;
    try {
      await unassignMut.mutateAsync({ clientUserId: Number(selClient), internalUserId: uid });
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Assign internal users to client accounts</p>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>Select Client</CardTitle>
          <CardDescription>Choose a client user</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex gap-2">
              <Select value={selInternal} onValueChange={setSelInternal}>
              ...
              <Button onClick={handleAssign} disabled={!selInternal}>
                ...
                {assignments.map((a) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline">#{a.internal_user_id}</Badge>
                    </div>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-6 w-6"
                      onClick={() => handleUnassign(a.internal_user_id)}
                    >
