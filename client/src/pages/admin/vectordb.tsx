...
const METRIC_INFO: Record<string, string> = {
  ...
  optimizer:
    "Whether the background optimizer is running normally. If 'error' — manual action may be needed.",
  points:
    "Total number of documents (chunks) stored. Each chunk = one point with a vector and payload.",
  vectors: "Total vectors available for search. Equals points_count when no deletions occurred.",
  indexed:
    "Vectors that have been indexed by HNSW and are ready for fast approximate search. Should equal vectors_count when indexing is complete.",
  segments:
    "Number of segments the collection is split into. More segments = more parallel search. Optimal range: 1–4 for most use cases.",
  "vector size":
    "Dimensionality of the embedding vector (e.g. 1024 for bge-m3). All vectors in a collection must share the same size.",
  distance:
    "Similarity metric used for vector comparison. 'cosine' for normalized embeddings, 'dot' for raw scores, 'euclid' for L2 distance.",
  "hnsw (m / ef_construct)":
    "HNSW graph parameters: m = max connections per node (higher = more recall, more RAM), ef_construct = build-time beam width (higher = better graph quality, slower indexing).",
  "on disk payload":
    "When 'Yes', payloads are stored on disk instead of RAM. Saves memory at the cost of slightly slower query latency.",
};
                    ...
                    <TableBody>
                      <TableRow>
                        <TableCell className="font-medium">
                          <TooltipLabel label="Status" tooltip={METRIC_INFO.status} />
                        </TableCell>
                        <TableCell>
                      ...
                      <TableRow>
                        <TableCell className="font-medium">
                          <TooltipLabel label="Optimizer" tooltip={METRIC_INFO.optimizer} />
                        </TableCell>
                        <TableCell>
                          <Badge
                            variant={col.optimizer_status === "ok" ? "success" : "destructive"}
                          >
                      ...
                      <TableRow>
                        <TableCell className="font-medium">
                          <TooltipLabel label="Points" tooltip={METRIC_INFO.points} />
                        </TableCell>
                        <TableCell>{col.points_count.toLocaleString()}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell className="font-medium">
                          <TooltipLabel label="Vectors" tooltip={METRIC_INFO.vectors} />
                        </TableCell>
                        <TableCell>
                      ...
                      <TableRow>
                        <TableCell className="font-medium">
                          <TooltipLabel label="Indexed" tooltip={METRIC_INFO.indexed} />
                        </TableCell>
                        <TableCell>
                      ...
                      <TableRow>
                        <TableCell className="font-medium">
                          <TooltipLabel label="Segments" tooltip={METRIC_INFO.segments} />
                        </TableCell>
                        <TableCell>{col.segments_count}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell className="font-medium">
                          <TooltipLabel label="Vector Size" tooltip={METRIC_INFO["vector size"]} />
                        </TableCell>
                        <TableCell>{col.vector_size ?? "—"}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell className="font-medium">
                          <TooltipLabel label="Distance" tooltip={METRIC_INFO.distance} />
                        </TableCell>
                      ...
                      <TableRow>
                        <TableCell className="font-medium">
                          <TooltipLabel
                            label="HNSW (m / ef_construct)"
                            tooltip={METRIC_INFO["hnsw (m / ef_construct)"]}
                          />
                        </TableCell>
                        <TableCell>
                      ...
                      <TableRow>
                        <TableCell className="font-medium">
                          <TooltipLabel
                            ...
                            tooltip={METRIC_INFO["on disk payload"]}
                          />
                        </TableCell>
                        <TableCell>
                          <Badge variant={col.on_disk_payload ? "secondary" : "outline"}>
                            {col.on_disk_payload === true
                              ? "Yes"
                              : col.on_disk_payload === false
                                ? "No"
                                : "—"}
                          </Badge>
                        </TableCell>
          ...
          <Card>
            <CardContent className="pt-6">
              <p className="text-sm text-muted-foreground text-center py-8">
                {data?.qdrant_status !== "ok" ? "Cannot connect to Qdrant" : "No collections found"}
              </p>
            </CardContent>
          </Card>
        )}
